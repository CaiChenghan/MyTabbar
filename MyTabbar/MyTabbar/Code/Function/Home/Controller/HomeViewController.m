...
- (void)viewDidLoad {
    ...
    [_myTableView registerClass:[HotNewsCell class] forCellReuseIdentifier:@"HotNewsCell"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.numberOfLine = numberOfLine;
    return cell;
}
...
-(void)loadDataFromServer:(BOOL)refresh{
    ...
    [_dataArray addObjectsFromArray:[self createData]];
...
-(void)resetStyle{
    //随机生成一个行数
    numberOfLine = [self createNumberOfLine:numberOfLine];
    [_myTableView reloadData];
}

-(NSInteger)createNumberOfLine:(NSInteger)line{
    int x = arc4random() % 5;
    if (x == line) {
        return [self createNumberOfLine:line];
    }
    return x;
}
